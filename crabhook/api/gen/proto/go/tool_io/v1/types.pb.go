// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.8
// 	protoc        (unknown)
// source: tool_io/v1/types.proto

package tool_iov1

import (
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	structpb "google.golang.org/protobuf/types/known/structpb"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

// SettingSource represents where a setting comes from.
// Copied from https://platform.claude.com/docs/en/agent-sdk/typescript#setting-source
type SettingSource int32

const (
	SettingSource_SETTING_SOURCE_UNSPECIFIED SettingSource = 0
	SettingSource_SETTING_SOURCE_USER        SettingSource = 1
	SettingSource_SETTING_SOURCE_PROJECT     SettingSource = 2
	SettingSource_SETTING_SOURCE_LOCAL       SettingSource = 3
)

// Enum value maps for SettingSource.
var (
	SettingSource_name = map[int32]string{
		0: "SETTING_SOURCE_UNSPECIFIED",
		1: "SETTING_SOURCE_USER",
		2: "SETTING_SOURCE_PROJECT",
		3: "SETTING_SOURCE_LOCAL",
	}
	SettingSource_value = map[string]int32{
		"SETTING_SOURCE_UNSPECIFIED": 0,
		"SETTING_SOURCE_USER":        1,
		"SETTING_SOURCE_PROJECT":     2,
		"SETTING_SOURCE_LOCAL":       3,
	}
)

func (x SettingSource) Enum() *SettingSource {
	p := new(SettingSource)
	*p = x
	return p
}

func (x SettingSource) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (SettingSource) Descriptor() protoreflect.EnumDescriptor {
	return file_tool_io_v1_types_proto_enumTypes[0].Descriptor()
}

func (SettingSource) Type() protoreflect.EnumType {
	return &file_tool_io_v1_types_proto_enumTypes[0]
}

func (x SettingSource) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use SettingSource.Descriptor instead.
func (SettingSource) EnumDescriptor() ([]byte, []int) {
	return file_tool_io_v1_types_proto_rawDescGZIP(), []int{0}
}

// AgentModel represents the model to use for an agent.
// Copied from https://platform.claude.com/docs/en/agent-sdk/typescript#agent-definition
type AgentModel int32

const (
	AgentModel_AGENT_MODEL_UNSPECIFIED AgentModel = 0
	AgentModel_AGENT_MODEL_SONNET      AgentModel = 1
	AgentModel_AGENT_MODEL_OPUS        AgentModel = 2
	AgentModel_AGENT_MODEL_HAIKU       AgentModel = 3
	AgentModel_AGENT_MODEL_INHERIT     AgentModel = 4
)

// Enum value maps for AgentModel.
var (
	AgentModel_name = map[int32]string{
		0: "AGENT_MODEL_UNSPECIFIED",
		1: "AGENT_MODEL_SONNET",
		2: "AGENT_MODEL_OPUS",
		3: "AGENT_MODEL_HAIKU",
		4: "AGENT_MODEL_INHERIT",
	}
	AgentModel_value = map[string]int32{
		"AGENT_MODEL_UNSPECIFIED": 0,
		"AGENT_MODEL_SONNET":      1,
		"AGENT_MODEL_OPUS":        2,
		"AGENT_MODEL_HAIKU":       3,
		"AGENT_MODEL_INHERIT":     4,
	}
)

func (x AgentModel) Enum() *AgentModel {
	p := new(AgentModel)
	*p = x
	return p
}

func (x AgentModel) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (AgentModel) Descriptor() protoreflect.EnumDescriptor {
	return file_tool_io_v1_types_proto_enumTypes[1].Descriptor()
}

func (AgentModel) Type() protoreflect.EnumType {
	return &file_tool_io_v1_types_proto_enumTypes[1]
}

func (x AgentModel) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use AgentModel.Descriptor instead.
func (AgentModel) EnumDescriptor() ([]byte, []int) {
	return file_tool_io_v1_types_proto_rawDescGZIP(), []int{1}
}

// PermissionResult represents the result of a permission check.
// Copied from https://platform.claude.com/docs/en/agent-sdk/typescript#permission-result
type PermissionResult struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Types that are valid to be assigned to Result:
	//
	//	*PermissionResult_Allow
	//	*PermissionResult_Deny
	Result        isPermissionResult_Result `protobuf_oneof:"result"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *PermissionResult) Reset() {
	*x = PermissionResult{}
	mi := &file_tool_io_v1_types_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *PermissionResult) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PermissionResult) ProtoMessage() {}

func (x *PermissionResult) ProtoReflect() protoreflect.Message {
	mi := &file_tool_io_v1_types_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use PermissionResult.ProtoReflect.Descriptor instead.
func (*PermissionResult) Descriptor() ([]byte, []int) {
	return file_tool_io_v1_types_proto_rawDescGZIP(), []int{0}
}

func (x *PermissionResult) GetResult() isPermissionResult_Result {
	if x != nil {
		return x.Result
	}
	return nil
}

func (x *PermissionResult) GetAllow() *AllowResult {
	if x != nil {
		if x, ok := x.Result.(*PermissionResult_Allow); ok {
			return x.Allow
		}
	}
	return nil
}

func (x *PermissionResult) GetDeny() *DenyResult {
	if x != nil {
		if x, ok := x.Result.(*PermissionResult_Deny); ok {
			return x.Deny
		}
	}
	return nil
}

type isPermissionResult_Result interface {
	isPermissionResult_Result()
}

type PermissionResult_Allow struct {
	Allow *AllowResult `protobuf:"bytes,1,opt,name=allow,proto3,oneof"`
}

type PermissionResult_Deny struct {
	Deny *DenyResult `protobuf:"bytes,2,opt,name=deny,proto3,oneof"`
}

func (*PermissionResult_Allow) isPermissionResult_Result() {}

func (*PermissionResult_Deny) isPermissionResult_Result() {}

// AllowResult represents an allowed permission result.
type AllowResult struct {
	state              protoimpl.MessageState `protogen:"open.v1"`
	UpdatedInput       *ToolInput             `protobuf:"bytes,1,opt,name=updated_input,json=updatedInput,proto3" json:"updated_input,omitempty"`
	UpdatedPermissions []*PermissionUpdate    `protobuf:"bytes,2,rep,name=updated_permissions,json=updatedPermissions,proto3" json:"updated_permissions,omitempty"`
	unknownFields      protoimpl.UnknownFields
	sizeCache          protoimpl.SizeCache
}

func (x *AllowResult) Reset() {
	*x = AllowResult{}
	mi := &file_tool_io_v1_types_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *AllowResult) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*AllowResult) ProtoMessage() {}

func (x *AllowResult) ProtoReflect() protoreflect.Message {
	mi := &file_tool_io_v1_types_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use AllowResult.ProtoReflect.Descriptor instead.
func (*AllowResult) Descriptor() ([]byte, []int) {
	return file_tool_io_v1_types_proto_rawDescGZIP(), []int{1}
}

func (x *AllowResult) GetUpdatedInput() *ToolInput {
	if x != nil {
		return x.UpdatedInput
	}
	return nil
}

func (x *AllowResult) GetUpdatedPermissions() []*PermissionUpdate {
	if x != nil {
		return x.UpdatedPermissions
	}
	return nil
}

// DenyResult represents a denied permission result.
type DenyResult struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Message       string                 `protobuf:"bytes,1,opt,name=message,proto3" json:"message,omitempty"`
	Interrupt     *bool                  `protobuf:"varint,2,opt,name=interrupt,proto3,oneof" json:"interrupt,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DenyResult) Reset() {
	*x = DenyResult{}
	mi := &file_tool_io_v1_types_proto_msgTypes[2]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DenyResult) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DenyResult) ProtoMessage() {}

func (x *DenyResult) ProtoReflect() protoreflect.Message {
	mi := &file_tool_io_v1_types_proto_msgTypes[2]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DenyResult.ProtoReflect.Descriptor instead.
func (*DenyResult) Descriptor() ([]byte, []int) {
	return file_tool_io_v1_types_proto_rawDescGZIP(), []int{2}
}

func (x *DenyResult) GetMessage() string {
	if x != nil {
		return x.Message
	}
	return ""
}

func (x *DenyResult) GetInterrupt() bool {
	if x != nil && x.Interrupt != nil {
		return *x.Interrupt
	}
	return false
}

// McpServerConfig represents configuration for an MCP server.
// Copied from https://platform.claude.com/docs/en/agent-sdk/typescript#mcp-server-config
type McpServerConfig struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Types that are valid to be assigned to Config:
	//
	//	*McpServerConfig_Stdio
	//	*McpServerConfig_Sse
	//	*McpServerConfig_Http
	Config        isMcpServerConfig_Config `protobuf_oneof:"config"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *McpServerConfig) Reset() {
	*x = McpServerConfig{}
	mi := &file_tool_io_v1_types_proto_msgTypes[3]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *McpServerConfig) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*McpServerConfig) ProtoMessage() {}

func (x *McpServerConfig) ProtoReflect() protoreflect.Message {
	mi := &file_tool_io_v1_types_proto_msgTypes[3]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use McpServerConfig.ProtoReflect.Descriptor instead.
func (*McpServerConfig) Descriptor() ([]byte, []int) {
	return file_tool_io_v1_types_proto_rawDescGZIP(), []int{3}
}

func (x *McpServerConfig) GetConfig() isMcpServerConfig_Config {
	if x != nil {
		return x.Config
	}
	return nil
}

func (x *McpServerConfig) GetStdio() *McpStdioServerConfig {
	if x != nil {
		if x, ok := x.Config.(*McpServerConfig_Stdio); ok {
			return x.Stdio
		}
	}
	return nil
}

func (x *McpServerConfig) GetSse() *McpSSEServerConfig {
	if x != nil {
		if x, ok := x.Config.(*McpServerConfig_Sse); ok {
			return x.Sse
		}
	}
	return nil
}

func (x *McpServerConfig) GetHttp() *McpHttpServerConfig {
	if x != nil {
		if x, ok := x.Config.(*McpServerConfig_Http); ok {
			return x.Http
		}
	}
	return nil
}

type isMcpServerConfig_Config interface {
	isMcpServerConfig_Config()
}

type McpServerConfig_Stdio struct {
	Stdio *McpStdioServerConfig `protobuf:"bytes,1,opt,name=stdio,proto3,oneof"`
}

type McpServerConfig_Sse struct {
	Sse *McpSSEServerConfig `protobuf:"bytes,2,opt,name=sse,proto3,oneof"`
}

type McpServerConfig_Http struct {
	Http *McpHttpServerConfig `protobuf:"bytes,3,opt,name=http,proto3,oneof"`
}

func (*McpServerConfig_Stdio) isMcpServerConfig_Config() {}

func (*McpServerConfig_Sse) isMcpServerConfig_Config() {}

func (*McpServerConfig_Http) isMcpServerConfig_Config() {}

// McpStdioServerConfig represents configuration for an MCP server using stdio transport.
type McpStdioServerConfig struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Command       string                 `protobuf:"bytes,1,opt,name=command,proto3" json:"command,omitempty"`
	Args          []string               `protobuf:"bytes,2,rep,name=args,proto3" json:"args,omitempty"`
	Env           map[string]string      `protobuf:"bytes,3,rep,name=env,proto3" json:"env,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *McpStdioServerConfig) Reset() {
	*x = McpStdioServerConfig{}
	mi := &file_tool_io_v1_types_proto_msgTypes[4]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *McpStdioServerConfig) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*McpStdioServerConfig) ProtoMessage() {}

func (x *McpStdioServerConfig) ProtoReflect() protoreflect.Message {
	mi := &file_tool_io_v1_types_proto_msgTypes[4]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use McpStdioServerConfig.ProtoReflect.Descriptor instead.
func (*McpStdioServerConfig) Descriptor() ([]byte, []int) {
	return file_tool_io_v1_types_proto_rawDescGZIP(), []int{4}
}

func (x *McpStdioServerConfig) GetCommand() string {
	if x != nil {
		return x.Command
	}
	return ""
}

func (x *McpStdioServerConfig) GetArgs() []string {
	if x != nil {
		return x.Args
	}
	return nil
}

func (x *McpStdioServerConfig) GetEnv() map[string]string {
	if x != nil {
		return x.Env
	}
	return nil
}

// McpSSEServerConfig represents configuration for an MCP server using SSE transport.
type McpSSEServerConfig struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Url           string                 `protobuf:"bytes,1,opt,name=url,proto3" json:"url,omitempty"`
	Headers       map[string]string      `protobuf:"bytes,2,rep,name=headers,proto3" json:"headers,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *McpSSEServerConfig) Reset() {
	*x = McpSSEServerConfig{}
	mi := &file_tool_io_v1_types_proto_msgTypes[5]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *McpSSEServerConfig) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*McpSSEServerConfig) ProtoMessage() {}

func (x *McpSSEServerConfig) ProtoReflect() protoreflect.Message {
	mi := &file_tool_io_v1_types_proto_msgTypes[5]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use McpSSEServerConfig.ProtoReflect.Descriptor instead.
func (*McpSSEServerConfig) Descriptor() ([]byte, []int) {
	return file_tool_io_v1_types_proto_rawDescGZIP(), []int{5}
}

func (x *McpSSEServerConfig) GetUrl() string {
	if x != nil {
		return x.Url
	}
	return ""
}

func (x *McpSSEServerConfig) GetHeaders() map[string]string {
	if x != nil {
		return x.Headers
	}
	return nil
}

// McpHttpServerConfig represents configuration for an MCP server using HTTP transport.
type McpHttpServerConfig struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Url           string                 `protobuf:"bytes,1,opt,name=url,proto3" json:"url,omitempty"`
	Headers       map[string]string      `protobuf:"bytes,2,rep,name=headers,proto3" json:"headers,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *McpHttpServerConfig) Reset() {
	*x = McpHttpServerConfig{}
	mi := &file_tool_io_v1_types_proto_msgTypes[6]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *McpHttpServerConfig) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*McpHttpServerConfig) ProtoMessage() {}

func (x *McpHttpServerConfig) ProtoReflect() protoreflect.Message {
	mi := &file_tool_io_v1_types_proto_msgTypes[6]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use McpHttpServerConfig.ProtoReflect.Descriptor instead.
func (*McpHttpServerConfig) Descriptor() ([]byte, []int) {
	return file_tool_io_v1_types_proto_rawDescGZIP(), []int{6}
}

func (x *McpHttpServerConfig) GetUrl() string {
	if x != nil {
		return x.Url
	}
	return ""
}

func (x *McpHttpServerConfig) GetHeaders() map[string]string {
	if x != nil {
		return x.Headers
	}
	return nil
}

// SdkPluginConfig represents configuration for an SDK plugin.
// Copied from https://platform.claude.com/docs/en/agent-sdk/typescript#sdk-plugin-config
type SdkPluginConfig struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Path          string                 `protobuf:"bytes,1,opt,name=path,proto3" json:"path,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SdkPluginConfig) Reset() {
	*x = SdkPluginConfig{}
	mi := &file_tool_io_v1_types_proto_msgTypes[7]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SdkPluginConfig) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SdkPluginConfig) ProtoMessage() {}

func (x *SdkPluginConfig) ProtoReflect() protoreflect.Message {
	mi := &file_tool_io_v1_types_proto_msgTypes[7]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SdkPluginConfig.ProtoReflect.Descriptor instead.
func (*SdkPluginConfig) Descriptor() ([]byte, []int) {
	return file_tool_io_v1_types_proto_rawDescGZIP(), []int{7}
}

func (x *SdkPluginConfig) GetPath() string {
	if x != nil {
		return x.Path
	}
	return ""
}

// AgentDefinition represents the definition of a sub-agent.
// Copied from https://platform.claude.com/docs/en/agent-sdk/typescript#agent-definition
type AgentDefinition struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Description   string                 `protobuf:"bytes,1,opt,name=description,proto3" json:"description,omitempty"`
	Tools         []string               `protobuf:"bytes,2,rep,name=tools,proto3" json:"tools,omitempty"`
	Prompt        string                 `protobuf:"bytes,3,opt,name=prompt,proto3" json:"prompt,omitempty"`
	Model         AgentModel             `protobuf:"varint,4,opt,name=model,proto3,enum=tool_io.v1.AgentModel" json:"model,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *AgentDefinition) Reset() {
	*x = AgentDefinition{}
	mi := &file_tool_io_v1_types_proto_msgTypes[8]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *AgentDefinition) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*AgentDefinition) ProtoMessage() {}

func (x *AgentDefinition) ProtoReflect() protoreflect.Message {
	mi := &file_tool_io_v1_types_proto_msgTypes[8]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use AgentDefinition.ProtoReflect.Descriptor instead.
func (*AgentDefinition) Descriptor() ([]byte, []int) {
	return file_tool_io_v1_types_proto_rawDescGZIP(), []int{8}
}

func (x *AgentDefinition) GetDescription() string {
	if x != nil {
		return x.Description
	}
	return ""
}

func (x *AgentDefinition) GetTools() []string {
	if x != nil {
		return x.Tools
	}
	return nil
}

func (x *AgentDefinition) GetPrompt() string {
	if x != nil {
		return x.Prompt
	}
	return ""
}

func (x *AgentDefinition) GetModel() AgentModel {
	if x != nil {
		return x.Model
	}
	return AgentModel_AGENT_MODEL_UNSPECIFIED
}

// SystemPromptPreset represents a preset system prompt with appended text.
// Copied from https://platform.claude.com/docs/en/agent-sdk/typescript#system-prompt
type SystemPromptPreset struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Append        string                 `protobuf:"bytes,1,opt,name=append,proto3" json:"append,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SystemPromptPreset) Reset() {
	*x = SystemPromptPreset{}
	mi := &file_tool_io_v1_types_proto_msgTypes[9]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SystemPromptPreset) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SystemPromptPreset) ProtoMessage() {}

func (x *SystemPromptPreset) ProtoReflect() protoreflect.Message {
	mi := &file_tool_io_v1_types_proto_msgTypes[9]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SystemPromptPreset.ProtoReflect.Descriptor instead.
func (*SystemPromptPreset) Descriptor() ([]byte, []int) {
	return file_tool_io_v1_types_proto_rawDescGZIP(), []int{9}
}

func (x *SystemPromptPreset) GetAppend() string {
	if x != nil {
		return x.Append
	}
	return ""
}

// SystemPrompt represents the system prompt configuration.
// Copied from https://platform.claude.com/docs/en/agent-sdk/typescript#system-prompt
type SystemPrompt struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Types that are valid to be assigned to Prompt:
	//
	//	*SystemPrompt_Custom
	//	*SystemPrompt_Preset
	Prompt        isSystemPrompt_Prompt `protobuf_oneof:"prompt"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SystemPrompt) Reset() {
	*x = SystemPrompt{}
	mi := &file_tool_io_v1_types_proto_msgTypes[10]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SystemPrompt) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SystemPrompt) ProtoMessage() {}

func (x *SystemPrompt) ProtoReflect() protoreflect.Message {
	mi := &file_tool_io_v1_types_proto_msgTypes[10]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SystemPrompt.ProtoReflect.Descriptor instead.
func (*SystemPrompt) Descriptor() ([]byte, []int) {
	return file_tool_io_v1_types_proto_rawDescGZIP(), []int{10}
}

func (x *SystemPrompt) GetPrompt() isSystemPrompt_Prompt {
	if x != nil {
		return x.Prompt
	}
	return nil
}

func (x *SystemPrompt) GetCustom() string {
	if x != nil {
		if x, ok := x.Prompt.(*SystemPrompt_Custom); ok {
			return x.Custom
		}
	}
	return ""
}

func (x *SystemPrompt) GetPreset() *SystemPromptPreset {
	if x != nil {
		if x, ok := x.Prompt.(*SystemPrompt_Preset); ok {
			return x.Preset
		}
	}
	return nil
}

type isSystemPrompt_Prompt interface {
	isSystemPrompt_Prompt()
}

type SystemPrompt_Custom struct {
	Custom string `protobuf:"bytes,1,opt,name=custom,proto3,oneof"`
}

type SystemPrompt_Preset struct {
	Preset *SystemPromptPreset `protobuf:"bytes,2,opt,name=preset,proto3,oneof"`
}

func (*SystemPrompt_Custom) isSystemPrompt_Prompt() {}

func (*SystemPrompt_Preset) isSystemPrompt_Prompt() {}

// OutputFormat represents the output format configuration with a JSON schema.
// Copied from https://platform.claude.com/docs/en/agent-sdk/typescript#output-format
type OutputFormat struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Schema        *structpb.Struct       `protobuf:"bytes,1,opt,name=schema,proto3" json:"schema,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *OutputFormat) Reset() {
	*x = OutputFormat{}
	mi := &file_tool_io_v1_types_proto_msgTypes[11]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *OutputFormat) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*OutputFormat) ProtoMessage() {}

func (x *OutputFormat) ProtoReflect() protoreflect.Message {
	mi := &file_tool_io_v1_types_proto_msgTypes[11]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use OutputFormat.ProtoReflect.Descriptor instead.
func (*OutputFormat) Descriptor() ([]byte, []int) {
	return file_tool_io_v1_types_proto_rawDescGZIP(), []int{11}
}

func (x *OutputFormat) GetSchema() *structpb.Struct {
	if x != nil {
		return x.Schema
	}
	return nil
}

// Options represents the configuration options for the SDK.
// Copied from https://platform.claude.com/docs/en/agent-sdk/typescript#options
// Note: Runtime-only fields (abortController, canUseTool, stderr, hooks) are omitted.
type Options struct {
	state                           protoimpl.MessageState      `protogen:"open.v1"`
	AdditionalDirectories           []string                    `protobuf:"bytes,1,rep,name=additional_directories,json=additionalDirectories,proto3" json:"additional_directories,omitempty"`
	Agents                          map[string]*AgentDefinition `protobuf:"bytes,2,rep,name=agents,proto3" json:"agents,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	AllowDangerouslySkipPermissions bool                        `protobuf:"varint,3,opt,name=allow_dangerously_skip_permissions,json=allowDangerouslySkipPermissions,proto3" json:"allow_dangerously_skip_permissions,omitempty"`
	AllowedTools                    []string                    `protobuf:"bytes,4,rep,name=allowed_tools,json=allowedTools,proto3" json:"allowed_tools,omitempty"`
	Betas                           []SdkBeta                   `protobuf:"varint,5,rep,packed,name=betas,proto3,enum=tool_io.v1.SdkBeta" json:"betas,omitempty"`
	Continue                        bool                        `protobuf:"varint,6,opt,name=continue,proto3" json:"continue,omitempty"`
	Cwd                             string                      `protobuf:"bytes,7,opt,name=cwd,proto3" json:"cwd,omitempty"`
	DisallowedTools                 []string                    `protobuf:"bytes,8,rep,name=disallowed_tools,json=disallowedTools,proto3" json:"disallowed_tools,omitempty"`
	EnableFileCheckpointing         bool                        `protobuf:"varint,9,opt,name=enable_file_checkpointing,json=enableFileCheckpointing,proto3" json:"enable_file_checkpointing,omitempty"`
	Env                             map[string]string           `protobuf:"bytes,10,rep,name=env,proto3" json:"env,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	Executable                      string                      `protobuf:"bytes,11,opt,name=executable,proto3" json:"executable,omitempty"`
	ExecutableArgs                  []string                    `protobuf:"bytes,12,rep,name=executable_args,json=executableArgs,proto3" json:"executable_args,omitempty"`
	ExtraArgs                       map[string]string           `protobuf:"bytes,13,rep,name=extra_args,json=extraArgs,proto3" json:"extra_args,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	FallbackModel                   string                      `protobuf:"bytes,14,opt,name=fallback_model,json=fallbackModel,proto3" json:"fallback_model,omitempty"`
	ForkSession                     bool                        `protobuf:"varint,15,opt,name=fork_session,json=forkSession,proto3" json:"fork_session,omitempty"`
	IncludePartialMessages          bool                        `protobuf:"varint,16,opt,name=include_partial_messages,json=includePartialMessages,proto3" json:"include_partial_messages,omitempty"`
	MaxBudgetUsd                    *float64                    `protobuf:"fixed64,17,opt,name=max_budget_usd,json=maxBudgetUsd,proto3,oneof" json:"max_budget_usd,omitempty"`
	MaxThinkingTokens               *int32                      `protobuf:"varint,18,opt,name=max_thinking_tokens,json=maxThinkingTokens,proto3,oneof" json:"max_thinking_tokens,omitempty"`
	MaxTurns                        *int32                      `protobuf:"varint,19,opt,name=max_turns,json=maxTurns,proto3,oneof" json:"max_turns,omitempty"`
	McpServers                      map[string]*McpServerConfig `protobuf:"bytes,20,rep,name=mcp_servers,json=mcpServers,proto3" json:"mcp_servers,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	Model                           string                      `protobuf:"bytes,21,opt,name=model,proto3" json:"model,omitempty"`
	OutputFormat                    *OutputFormat               `protobuf:"bytes,22,opt,name=output_format,json=outputFormat,proto3,oneof" json:"output_format,omitempty"`
	PathToClaudeCodeExecutable      string                      `protobuf:"bytes,23,opt,name=path_to_claude_code_executable,json=pathToClaudeCodeExecutable,proto3" json:"path_to_claude_code_executable,omitempty"`
	PermissionMode                  PermissionMode              `protobuf:"varint,24,opt,name=permission_mode,json=permissionMode,proto3,enum=tool_io.v1.PermissionMode" json:"permission_mode,omitempty"`
	PermissionPromptToolName        string                      `protobuf:"bytes,25,opt,name=permission_prompt_tool_name,json=permissionPromptToolName,proto3" json:"permission_prompt_tool_name,omitempty"`
	Plugins                         []*SdkPluginConfig          `protobuf:"bytes,26,rep,name=plugins,proto3" json:"plugins,omitempty"`
	Resume                          string                      `protobuf:"bytes,27,opt,name=resume,proto3" json:"resume,omitempty"`
	ResumeSessionAt                 string                      `protobuf:"bytes,28,opt,name=resume_session_at,json=resumeSessionAt,proto3" json:"resume_session_at,omitempty"`
	Sandbox                         *SandboxSettings            `protobuf:"bytes,29,opt,name=sandbox,proto3,oneof" json:"sandbox,omitempty"`
	SettingSources                  []SettingSource             `protobuf:"varint,30,rep,packed,name=setting_sources,json=settingSources,proto3,enum=tool_io.v1.SettingSource" json:"setting_sources,omitempty"`
	StrictMcpConfig                 bool                        `protobuf:"varint,31,opt,name=strict_mcp_config,json=strictMcpConfig,proto3" json:"strict_mcp_config,omitempty"`
	SystemPrompt                    *SystemPrompt               `protobuf:"bytes,32,opt,name=system_prompt,json=systemPrompt,proto3,oneof" json:"system_prompt,omitempty"`
	ToolsList                       []string                    `protobuf:"bytes,33,rep,name=tools_list,json=toolsList,proto3" json:"tools_list,omitempty"`
	ToolsPreset                     bool                        `protobuf:"varint,34,opt,name=tools_preset,json=toolsPreset,proto3" json:"tools_preset,omitempty"`
	unknownFields                   protoimpl.UnknownFields
	sizeCache                       protoimpl.SizeCache
}

func (x *Options) Reset() {
	*x = Options{}
	mi := &file_tool_io_v1_types_proto_msgTypes[12]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Options) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Options) ProtoMessage() {}

func (x *Options) ProtoReflect() protoreflect.Message {
	mi := &file_tool_io_v1_types_proto_msgTypes[12]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Options.ProtoReflect.Descriptor instead.
func (*Options) Descriptor() ([]byte, []int) {
	return file_tool_io_v1_types_proto_rawDescGZIP(), []int{12}
}

func (x *Options) GetAdditionalDirectories() []string {
	if x != nil {
		return x.AdditionalDirectories
	}
	return nil
}

func (x *Options) GetAgents() map[string]*AgentDefinition {
	if x != nil {
		return x.Agents
	}
	return nil
}

func (x *Options) GetAllowDangerouslySkipPermissions() bool {
	if x != nil {
		return x.AllowDangerouslySkipPermissions
	}
	return false
}

func (x *Options) GetAllowedTools() []string {
	if x != nil {
		return x.AllowedTools
	}
	return nil
}

func (x *Options) GetBetas() []SdkBeta {
	if x != nil {
		return x.Betas
	}
	return nil
}

func (x *Options) GetContinue() bool {
	if x != nil {
		return x.Continue
	}
	return false
}

func (x *Options) GetCwd() string {
	if x != nil {
		return x.Cwd
	}
	return ""
}

func (x *Options) GetDisallowedTools() []string {
	if x != nil {
		return x.DisallowedTools
	}
	return nil
}

func (x *Options) GetEnableFileCheckpointing() bool {
	if x != nil {
		return x.EnableFileCheckpointing
	}
	return false
}

func (x *Options) GetEnv() map[string]string {
	if x != nil {
		return x.Env
	}
	return nil
}

func (x *Options) GetExecutable() string {
	if x != nil {
		return x.Executable
	}
	return ""
}

func (x *Options) GetExecutableArgs() []string {
	if x != nil {
		return x.ExecutableArgs
	}
	return nil
}

func (x *Options) GetExtraArgs() map[string]string {
	if x != nil {
		return x.ExtraArgs
	}
	return nil
}

func (x *Options) GetFallbackModel() string {
	if x != nil {
		return x.FallbackModel
	}
	return ""
}

func (x *Options) GetForkSession() bool {
	if x != nil {
		return x.ForkSession
	}
	return false
}

func (x *Options) GetIncludePartialMessages() bool {
	if x != nil {
		return x.IncludePartialMessages
	}
	return false
}

func (x *Options) GetMaxBudgetUsd() float64 {
	if x != nil && x.MaxBudgetUsd != nil {
		return *x.MaxBudgetUsd
	}
	return 0
}

func (x *Options) GetMaxThinkingTokens() int32 {
	if x != nil && x.MaxThinkingTokens != nil {
		return *x.MaxThinkingTokens
	}
	return 0
}

func (x *Options) GetMaxTurns() int32 {
	if x != nil && x.MaxTurns != nil {
		return *x.MaxTurns
	}
	return 0
}

func (x *Options) GetMcpServers() map[string]*McpServerConfig {
	if x != nil {
		return x.McpServers
	}
	return nil
}

func (x *Options) GetModel() string {
	if x != nil {
		return x.Model
	}
	return ""
}

func (x *Options) GetOutputFormat() *OutputFormat {
	if x != nil {
		return x.OutputFormat
	}
	return nil
}

func (x *Options) GetPathToClaudeCodeExecutable() string {
	if x != nil {
		return x.PathToClaudeCodeExecutable
	}
	return ""
}

func (x *Options) GetPermissionMode() PermissionMode {
	if x != nil {
		return x.PermissionMode
	}
	return PermissionMode_PERMISSION_MODE_UNSPECIFIED
}

func (x *Options) GetPermissionPromptToolName() string {
	if x != nil {
		return x.PermissionPromptToolName
	}
	return ""
}

func (x *Options) GetPlugins() []*SdkPluginConfig {
	if x != nil {
		return x.Plugins
	}
	return nil
}

func (x *Options) GetResume() string {
	if x != nil {
		return x.Resume
	}
	return ""
}

func (x *Options) GetResumeSessionAt() string {
	if x != nil {
		return x.ResumeSessionAt
	}
	return ""
}

func (x *Options) GetSandbox() *SandboxSettings {
	if x != nil {
		return x.Sandbox
	}
	return nil
}

func (x *Options) GetSettingSources() []SettingSource {
	if x != nil {
		return x.SettingSources
	}
	return nil
}

func (x *Options) GetStrictMcpConfig() bool {
	if x != nil {
		return x.StrictMcpConfig
	}
	return false
}

func (x *Options) GetSystemPrompt() *SystemPrompt {
	if x != nil {
		return x.SystemPrompt
	}
	return nil
}

func (x *Options) GetToolsList() []string {
	if x != nil {
		return x.ToolsList
	}
	return nil
}

func (x *Options) GetToolsPreset() bool {
	if x != nil {
		return x.ToolsPreset
	}
	return false
}

var File_tool_io_v1_types_proto protoreflect.FileDescriptor

const file_tool_io_v1_types_proto_rawDesc = "" +
	"\n" +
	"\x16tool_io/v1/types.proto\x12\n" +
	"tool_io.v1\x1a\x1btool_io/v1/tool_input.proto\x1a\x1btool_io/v1/permission.proto\x1a\x18tool_io/v1/sandbox.proto\x1a\x16tool_io/v1/other.proto\x1a\x1cgoogle/protobuf/struct.proto\"{\n" +
	"\x10PermissionResult\x12/\n" +
	"\x05allow\x18\x01 \x01(\v2\x17.tool_io.v1.AllowResultH\x00R\x05allow\x12,\n" +
	"\x04deny\x18\x02 \x01(\v2\x16.tool_io.v1.DenyResultH\x00R\x04denyB\b\n" +
	"\x06result\"\x98\x01\n" +
	"\vAllowResult\x12:\n" +
	"\rupdated_input\x18\x01 \x01(\v2\x15.tool_io.v1.ToolInputR\fupdatedInput\x12M\n" +
	"\x13updated_permissions\x18\x02 \x03(\v2\x1c.tool_io.v1.PermissionUpdateR\x12updatedPermissions\"W\n" +
	"\n" +
	"DenyResult\x12\x18\n" +
	"\amessage\x18\x01 \x01(\tR\amessage\x12!\n" +
	"\tinterrupt\x18\x02 \x01(\bH\x00R\tinterrupt\x88\x01\x01B\f\n" +
	"\n" +
	"_interrupt\"\xc0\x01\n" +
	"\x0fMcpServerConfig\x128\n" +
	"\x05stdio\x18\x01 \x01(\v2 .tool_io.v1.McpStdioServerConfigH\x00R\x05stdio\x122\n" +
	"\x03sse\x18\x02 \x01(\v2\x1e.tool_io.v1.McpSSEServerConfigH\x00R\x03sse\x125\n" +
	"\x04http\x18\x03 \x01(\v2\x1f.tool_io.v1.McpHttpServerConfigH\x00R\x04httpB\b\n" +
	"\x06config\"\xb9\x01\n" +
	"\x14McpStdioServerConfig\x12\x18\n" +
	"\acommand\x18\x01 \x01(\tR\acommand\x12\x12\n" +
	"\x04args\x18\x02 \x03(\tR\x04args\x12;\n" +
	"\x03env\x18\x03 \x03(\v2).tool_io.v1.McpStdioServerConfig.EnvEntryR\x03env\x1a6\n" +
	"\bEnvEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x14\n" +
	"\x05value\x18\x02 \x01(\tR\x05value:\x028\x01\"\xa9\x01\n" +
	"\x12McpSSEServerConfig\x12\x10\n" +
	"\x03url\x18\x01 \x01(\tR\x03url\x12E\n" +
	"\aheaders\x18\x02 \x03(\v2+.tool_io.v1.McpSSEServerConfig.HeadersEntryR\aheaders\x1a:\n" +
	"\fHeadersEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x14\n" +
	"\x05value\x18\x02 \x01(\tR\x05value:\x028\x01\"\xab\x01\n" +
	"\x13McpHttpServerConfig\x12\x10\n" +
	"\x03url\x18\x01 \x01(\tR\x03url\x12F\n" +
	"\aheaders\x18\x02 \x03(\v2,.tool_io.v1.McpHttpServerConfig.HeadersEntryR\aheaders\x1a:\n" +
	"\fHeadersEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x14\n" +
	"\x05value\x18\x02 \x01(\tR\x05value:\x028\x01\"%\n" +
	"\x0fSdkPluginConfig\x12\x12\n" +
	"\x04path\x18\x01 \x01(\tR\x04path\"\x8f\x01\n" +
	"\x0fAgentDefinition\x12 \n" +
	"\vdescription\x18\x01 \x01(\tR\vdescription\x12\x14\n" +
	"\x05tools\x18\x02 \x03(\tR\x05tools\x12\x16\n" +
	"\x06prompt\x18\x03 \x01(\tR\x06prompt\x12,\n" +
	"\x05model\x18\x04 \x01(\x0e2\x16.tool_io.v1.AgentModelR\x05model\",\n" +
	"\x12SystemPromptPreset\x12\x16\n" +
	"\x06append\x18\x01 \x01(\tR\x06append\"l\n" +
	"\fSystemPrompt\x12\x18\n" +
	"\x06custom\x18\x01 \x01(\tH\x00R\x06custom\x128\n" +
	"\x06preset\x18\x02 \x01(\v2\x1e.tool_io.v1.SystemPromptPresetH\x00R\x06presetB\b\n" +
	"\x06prompt\"?\n" +
	"\fOutputFormat\x12/\n" +
	"\x06schema\x18\x01 \x01(\v2\x17.google.protobuf.StructR\x06schema\"\x95\x10\n" +
	"\aOptions\x125\n" +
	"\x16additional_directories\x18\x01 \x03(\tR\x15additionalDirectories\x127\n" +
	"\x06agents\x18\x02 \x03(\v2\x1f.tool_io.v1.Options.AgentsEntryR\x06agents\x12K\n" +
	"\"allow_dangerously_skip_permissions\x18\x03 \x01(\bR\x1fallowDangerouslySkipPermissions\x12#\n" +
	"\rallowed_tools\x18\x04 \x03(\tR\fallowedTools\x12)\n" +
	"\x05betas\x18\x05 \x03(\x0e2\x13.tool_io.v1.SdkBetaR\x05betas\x12\x1a\n" +
	"\bcontinue\x18\x06 \x01(\bR\bcontinue\x12\x10\n" +
	"\x03cwd\x18\a \x01(\tR\x03cwd\x12)\n" +
	"\x10disallowed_tools\x18\b \x03(\tR\x0fdisallowedTools\x12:\n" +
	"\x19enable_file_checkpointing\x18\t \x01(\bR\x17enableFileCheckpointing\x12.\n" +
	"\x03env\x18\n" +
	" \x03(\v2\x1c.tool_io.v1.Options.EnvEntryR\x03env\x12\x1e\n" +
	"\n" +
	"executable\x18\v \x01(\tR\n" +
	"executable\x12'\n" +
	"\x0fexecutable_args\x18\f \x03(\tR\x0eexecutableArgs\x12A\n" +
	"\n" +
	"extra_args\x18\r \x03(\v2\".tool_io.v1.Options.ExtraArgsEntryR\textraArgs\x12%\n" +
	"\x0efallback_model\x18\x0e \x01(\tR\rfallbackModel\x12!\n" +
	"\ffork_session\x18\x0f \x01(\bR\vforkSession\x128\n" +
	"\x18include_partial_messages\x18\x10 \x01(\bR\x16includePartialMessages\x12)\n" +
	"\x0emax_budget_usd\x18\x11 \x01(\x01H\x00R\fmaxBudgetUsd\x88\x01\x01\x123\n" +
	"\x13max_thinking_tokens\x18\x12 \x01(\x05H\x01R\x11maxThinkingTokens\x88\x01\x01\x12 \n" +
	"\tmax_turns\x18\x13 \x01(\x05H\x02R\bmaxTurns\x88\x01\x01\x12D\n" +
	"\vmcp_servers\x18\x14 \x03(\v2#.tool_io.v1.Options.McpServersEntryR\n" +
	"mcpServers\x12\x14\n" +
	"\x05model\x18\x15 \x01(\tR\x05model\x12B\n" +
	"\routput_format\x18\x16 \x01(\v2\x18.tool_io.v1.OutputFormatH\x03R\foutputFormat\x88\x01\x01\x12B\n" +
	"\x1epath_to_claude_code_executable\x18\x17 \x01(\tR\x1apathToClaudeCodeExecutable\x12C\n" +
	"\x0fpermission_mode\x18\x18 \x01(\x0e2\x1a.tool_io.v1.PermissionModeR\x0epermissionMode\x12=\n" +
	"\x1bpermission_prompt_tool_name\x18\x19 \x01(\tR\x18permissionPromptToolName\x125\n" +
	"\aplugins\x18\x1a \x03(\v2\x1b.tool_io.v1.SdkPluginConfigR\aplugins\x12\x16\n" +
	"\x06resume\x18\x1b \x01(\tR\x06resume\x12*\n" +
	"\x11resume_session_at\x18\x1c \x01(\tR\x0fresumeSessionAt\x12:\n" +
	"\asandbox\x18\x1d \x01(\v2\x1b.tool_io.v1.SandboxSettingsH\x04R\asandbox\x88\x01\x01\x12B\n" +
	"\x0fsetting_sources\x18\x1e \x03(\x0e2\x19.tool_io.v1.SettingSourceR\x0esettingSources\x12*\n" +
	"\x11strict_mcp_config\x18\x1f \x01(\bR\x0fstrictMcpConfig\x12B\n" +
	"\rsystem_prompt\x18  \x01(\v2\x18.tool_io.v1.SystemPromptH\x05R\fsystemPrompt\x88\x01\x01\x12\x1d\n" +
	"\n" +
	"tools_list\x18! \x03(\tR\ttoolsList\x12!\n" +
	"\ftools_preset\x18\" \x01(\bR\vtoolsPreset\x1aV\n" +
	"\vAgentsEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x121\n" +
	"\x05value\x18\x02 \x01(\v2\x1b.tool_io.v1.AgentDefinitionR\x05value:\x028\x01\x1a6\n" +
	"\bEnvEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x14\n" +
	"\x05value\x18\x02 \x01(\tR\x05value:\x028\x01\x1a<\n" +
	"\x0eExtraArgsEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x14\n" +
	"\x05value\x18\x02 \x01(\tR\x05value:\x028\x01\x1aZ\n" +
	"\x0fMcpServersEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x121\n" +
	"\x05value\x18\x02 \x01(\v2\x1b.tool_io.v1.McpServerConfigR\x05value:\x028\x01B\x11\n" +
	"\x0f_max_budget_usdB\x16\n" +
	"\x14_max_thinking_tokensB\f\n" +
	"\n" +
	"_max_turnsB\x10\n" +
	"\x0e_output_formatB\n" +
	"\n" +
	"\b_sandboxB\x10\n" +
	"\x0e_system_prompt*~\n" +
	"\rSettingSource\x12\x1e\n" +
	"\x1aSETTING_SOURCE_UNSPECIFIED\x10\x00\x12\x17\n" +
	"\x13SETTING_SOURCE_USER\x10\x01\x12\x1a\n" +
	"\x16SETTING_SOURCE_PROJECT\x10\x02\x12\x18\n" +
	"\x14SETTING_SOURCE_LOCAL\x10\x03*\x87\x01\n" +
	"\n" +
	"AgentModel\x12\x1b\n" +
	"\x17AGENT_MODEL_UNSPECIFIED\x10\x00\x12\x16\n" +
	"\x12AGENT_MODEL_SONNET\x10\x01\x12\x14\n" +
	"\x10AGENT_MODEL_OPUS\x10\x02\x12\x15\n" +
	"\x11AGENT_MODEL_HAIKU\x10\x03\x12\x17\n" +
	"\x13AGENT_MODEL_INHERIT\x10\x04B\xad\x01\n" +
	"\x0ecom.tool_io.v1B\n" +
	"TypesProtoP\x01ZJgithub.com/ngicks/crabswarm/crabhook/api/gen/proto/go/tool_io/v1;tool_iov1\xa2\x02\x03TXX\xaa\x02\tToolIo.V1\xca\x02\tToolIo\\V1\xe2\x02\x15ToolIo\\V1\\GPBMetadata\xea\x02\n" +
	"ToolIo::V1b\x06proto3"

var (
	file_tool_io_v1_types_proto_rawDescOnce sync.Once
	file_tool_io_v1_types_proto_rawDescData []byte
)

func file_tool_io_v1_types_proto_rawDescGZIP() []byte {
	file_tool_io_v1_types_proto_rawDescOnce.Do(func() {
		file_tool_io_v1_types_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_tool_io_v1_types_proto_rawDesc), len(file_tool_io_v1_types_proto_rawDesc)))
	})
	return file_tool_io_v1_types_proto_rawDescData
}

var file_tool_io_v1_types_proto_enumTypes = make([]protoimpl.EnumInfo, 2)
var file_tool_io_v1_types_proto_msgTypes = make([]protoimpl.MessageInfo, 20)
var file_tool_io_v1_types_proto_goTypes = []any{
	(SettingSource)(0),           // 0: tool_io.v1.SettingSource
	(AgentModel)(0),              // 1: tool_io.v1.AgentModel
	(*PermissionResult)(nil),     // 2: tool_io.v1.PermissionResult
	(*AllowResult)(nil),          // 3: tool_io.v1.AllowResult
	(*DenyResult)(nil),           // 4: tool_io.v1.DenyResult
	(*McpServerConfig)(nil),      // 5: tool_io.v1.McpServerConfig
	(*McpStdioServerConfig)(nil), // 6: tool_io.v1.McpStdioServerConfig
	(*McpSSEServerConfig)(nil),   // 7: tool_io.v1.McpSSEServerConfig
	(*McpHttpServerConfig)(nil),  // 8: tool_io.v1.McpHttpServerConfig
	(*SdkPluginConfig)(nil),      // 9: tool_io.v1.SdkPluginConfig
	(*AgentDefinition)(nil),      // 10: tool_io.v1.AgentDefinition
	(*SystemPromptPreset)(nil),   // 11: tool_io.v1.SystemPromptPreset
	(*SystemPrompt)(nil),         // 12: tool_io.v1.SystemPrompt
	(*OutputFormat)(nil),         // 13: tool_io.v1.OutputFormat
	(*Options)(nil),              // 14: tool_io.v1.Options
	nil,                          // 15: tool_io.v1.McpStdioServerConfig.EnvEntry
	nil,                          // 16: tool_io.v1.McpSSEServerConfig.HeadersEntry
	nil,                          // 17: tool_io.v1.McpHttpServerConfig.HeadersEntry
	nil,                          // 18: tool_io.v1.Options.AgentsEntry
	nil,                          // 19: tool_io.v1.Options.EnvEntry
	nil,                          // 20: tool_io.v1.Options.ExtraArgsEntry
	nil,                          // 21: tool_io.v1.Options.McpServersEntry
	(*ToolInput)(nil),            // 22: tool_io.v1.ToolInput
	(*PermissionUpdate)(nil),     // 23: tool_io.v1.PermissionUpdate
	(*structpb.Struct)(nil),      // 24: google.protobuf.Struct
	(SdkBeta)(0),                 // 25: tool_io.v1.SdkBeta
	(PermissionMode)(0),          // 26: tool_io.v1.PermissionMode
	(*SandboxSettings)(nil),      // 27: tool_io.v1.SandboxSettings
}
var file_tool_io_v1_types_proto_depIdxs = []int32{
	3,  // 0: tool_io.v1.PermissionResult.allow:type_name -> tool_io.v1.AllowResult
	4,  // 1: tool_io.v1.PermissionResult.deny:type_name -> tool_io.v1.DenyResult
	22, // 2: tool_io.v1.AllowResult.updated_input:type_name -> tool_io.v1.ToolInput
	23, // 3: tool_io.v1.AllowResult.updated_permissions:type_name -> tool_io.v1.PermissionUpdate
	6,  // 4: tool_io.v1.McpServerConfig.stdio:type_name -> tool_io.v1.McpStdioServerConfig
	7,  // 5: tool_io.v1.McpServerConfig.sse:type_name -> tool_io.v1.McpSSEServerConfig
	8,  // 6: tool_io.v1.McpServerConfig.http:type_name -> tool_io.v1.McpHttpServerConfig
	15, // 7: tool_io.v1.McpStdioServerConfig.env:type_name -> tool_io.v1.McpStdioServerConfig.EnvEntry
	16, // 8: tool_io.v1.McpSSEServerConfig.headers:type_name -> tool_io.v1.McpSSEServerConfig.HeadersEntry
	17, // 9: tool_io.v1.McpHttpServerConfig.headers:type_name -> tool_io.v1.McpHttpServerConfig.HeadersEntry
	1,  // 10: tool_io.v1.AgentDefinition.model:type_name -> tool_io.v1.AgentModel
	11, // 11: tool_io.v1.SystemPrompt.preset:type_name -> tool_io.v1.SystemPromptPreset
	24, // 12: tool_io.v1.OutputFormat.schema:type_name -> google.protobuf.Struct
	18, // 13: tool_io.v1.Options.agents:type_name -> tool_io.v1.Options.AgentsEntry
	25, // 14: tool_io.v1.Options.betas:type_name -> tool_io.v1.SdkBeta
	19, // 15: tool_io.v1.Options.env:type_name -> tool_io.v1.Options.EnvEntry
	20, // 16: tool_io.v1.Options.extra_args:type_name -> tool_io.v1.Options.ExtraArgsEntry
	21, // 17: tool_io.v1.Options.mcp_servers:type_name -> tool_io.v1.Options.McpServersEntry
	13, // 18: tool_io.v1.Options.output_format:type_name -> tool_io.v1.OutputFormat
	26, // 19: tool_io.v1.Options.permission_mode:type_name -> tool_io.v1.PermissionMode
	9,  // 20: tool_io.v1.Options.plugins:type_name -> tool_io.v1.SdkPluginConfig
	27, // 21: tool_io.v1.Options.sandbox:type_name -> tool_io.v1.SandboxSettings
	0,  // 22: tool_io.v1.Options.setting_sources:type_name -> tool_io.v1.SettingSource
	12, // 23: tool_io.v1.Options.system_prompt:type_name -> tool_io.v1.SystemPrompt
	10, // 24: tool_io.v1.Options.AgentsEntry.value:type_name -> tool_io.v1.AgentDefinition
	5,  // 25: tool_io.v1.Options.McpServersEntry.value:type_name -> tool_io.v1.McpServerConfig
	26, // [26:26] is the sub-list for method output_type
	26, // [26:26] is the sub-list for method input_type
	26, // [26:26] is the sub-list for extension type_name
	26, // [26:26] is the sub-list for extension extendee
	0,  // [0:26] is the sub-list for field type_name
}

func init() { file_tool_io_v1_types_proto_init() }
func file_tool_io_v1_types_proto_init() {
	if File_tool_io_v1_types_proto != nil {
		return
	}
	file_tool_io_v1_tool_input_proto_init()
	file_tool_io_v1_permission_proto_init()
	file_tool_io_v1_sandbox_proto_init()
	file_tool_io_v1_other_proto_init()
	file_tool_io_v1_types_proto_msgTypes[0].OneofWrappers = []any{
		(*PermissionResult_Allow)(nil),
		(*PermissionResult_Deny)(nil),
	}
	file_tool_io_v1_types_proto_msgTypes[2].OneofWrappers = []any{}
	file_tool_io_v1_types_proto_msgTypes[3].OneofWrappers = []any{
		(*McpServerConfig_Stdio)(nil),
		(*McpServerConfig_Sse)(nil),
		(*McpServerConfig_Http)(nil),
	}
	file_tool_io_v1_types_proto_msgTypes[10].OneofWrappers = []any{
		(*SystemPrompt_Custom)(nil),
		(*SystemPrompt_Preset)(nil),
	}
	file_tool_io_v1_types_proto_msgTypes[12].OneofWrappers = []any{}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_tool_io_v1_types_proto_rawDesc), len(file_tool_io_v1_types_proto_rawDesc)),
			NumEnums:      2,
			NumMessages:   20,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_tool_io_v1_types_proto_goTypes,
		DependencyIndexes: file_tool_io_v1_types_proto_depIdxs,
		EnumInfos:         file_tool_io_v1_types_proto_enumTypes,
		MessageInfos:      file_tool_io_v1_types_proto_msgTypes,
	}.Build()
	File_tool_io_v1_types_proto = out.File
	file_tool_io_v1_types_proto_goTypes = nil
	file_tool_io_v1_types_proto_depIdxs = nil
}
