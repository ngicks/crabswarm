...
│   ├── crabswarm-mcp          wires a harness (claude / codex / opencode) into its chat room — skill + the `crabswarm mcp` server, plus Codex's hooks; `opencode.ts` in the skill dir is the OpenCode plugin.
│   │                          The Claude Code plugin ships no hooks at all: a mention reaches that member through the fakechat channel or the daemon's typed nudge, and its state comes off the agents listing its own server polls.
│   │                          `codex-hooks.json` delivers messages only. env_vars/`.mcp.json` env forward CRABSWARM_CLAUDE_CHANNEL, CRABSWARM_CODEX_APP_SERVER and FAKECHAT_PORT, the launch-time variables that let the server hand a mention to its harness itself.
...
├── pkg
│   ├── harnessctl  Reach a coding-agent harness from beside it: Detect picks the CLI off the MCP clientInfo name; claude.go + claudeagents.go (a POST to the fakechat plugin's loopback server, probed with GET / first, and the state feed built on `claude agents --json`), codex*.go (the app server the session is hosted by — delivery and the state feed both), opencode.go (a POST to the plugin's loopback relay).
...
- Where a member's state comes from differs by harness, and no harness reports through hooks any more: Claude Code reports from the agents listing (`claude agents --json`) that its own `crabswarm mcp` polls every 2s, matching its session by `CLAUDE_CODE_SESSION_ID` and reporting on change; Codex reports on its app-server feed; OpenCode reports from the plugin's in-process events. A bridge watching a feed also re-sends the last state that feed reported every 10s, so a restarted daemon hears it again. The daemon's screen poller is the fallback for a harness with no feed of its own, so it polls only members whose harness is `other` (`chat.screen_poll_interval` / `CRABSWARM_CHAT_SCREEN_POLL_INTERVAL`, 0 = 3s, negative = off).
