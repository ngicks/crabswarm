...
│   │                          CRABSWARM_CLAUDE_CHANNEL, CRABSWARM_CODEX_APP_SERVER and FAKECHAT_PORT, the launch-time variables that let the server hand a mention to its harness itself.
...
├── pkg
│   ├── harnessctl  Reach a coding-agent harness from beside it: Detect picks the CLI off the MCP clientInfo name; claude.go (a POST to the fakechat plugin's loopback server, probed with GET / first), codex*.go (the app server the session is hosted by — delivery and the state feed both), opencode.go (a POST to the plugin's loopback relay).
