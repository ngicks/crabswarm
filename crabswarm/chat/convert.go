...
// unspecified state is rejected rather than defaulted: a reporter that failed
// to fill it in must not silently mark its agent done, which is the one state a
