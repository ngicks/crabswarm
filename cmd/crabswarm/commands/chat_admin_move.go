...
func chatAdminMoveCmd(parent *cobra.Command, flags *chatFlags) {
	cmd := &cobra.Command{
...
"team/name", so a line of ` + "`chat admin list`" + ` can be pasted in. The
room is named separately because a name may repeat across rooms.`,
		Example: `  crabswarm chat admin move /work/proj backend/alice frontend \
    --identity ~/.config/crabswarm/chat_admin.key`,
		Args:              cobra.ExactArgs(3),
		ValidArgsFunction: cobra.NoFileCompletions,
		RunE: func(cmd *cobra.Command, args []string) error {
			return runChatAdminMove(cmd, args, flags)
		},
	}
...
func runChatAdminMove(cmd *cobra.Command, args []string, flags *chatFlags) error {
