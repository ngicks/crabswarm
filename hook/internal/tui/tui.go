...
func (m rootModel) View() string {
	...
	case statePermission:
		queued := ""
		if len(m.queuedReqs) > 0 {
			queued = statusBarStyle.Render(fmt.Sprintf("  %d request(s) queued", len(m.queuedReqs)))
		}
		return m.permModel.View() + queued
	case stateAskUser:
		queued := ""
		if len(m.queuedReqs) > 0 {
			queued = statusBarStyle.Render(fmt.Sprintf("  %d request(s) queued", len(m.queuedReqs)))
		}
		return m.askModel.View() + queued
	case stateExitPlan:
		queued := ""
		if len(m.queuedReqs) > 0 {
			queued = statusBarStyle.Render(fmt.Sprintf("  %d request(s) queued", len(m.queuedReqs)))
		}
		return m.exitModel.View() + queued
	default:
		return statusBarStyle.Render("  Waiting for permission requests...")
	}
}
