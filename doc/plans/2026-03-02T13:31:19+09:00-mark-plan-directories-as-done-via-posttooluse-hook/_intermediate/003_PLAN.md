...
- `Load()`: if `cached != nil`, return `*cached`. Otherwise read from disk, cache, return. If `state.json` does not exist, return zero-value `PlanDirState{}` (no error) and cache it — this handles new directories.
- `Update(fn)`: load if not cached (with the same missing-file-is-zero-value rule), call `fn(cached)`, marshal+write to disk, update cache.
...
- Add `TestPlanDirStateStore_LoadMissingFile` — `Load()` on nonexistent state.json returns zero-value, no error
- Add `TestPlanDirStateStore_UpdateBootstrap` — `Update` on new dir (no state.json) succeeds, creates file
