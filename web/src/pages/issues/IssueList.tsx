...
// The list: one table row per matching issue, newest updated first, with the
// affordances every issue gets when it has the data — an epic progress bar
// from child status, metadata chips. A plan looks like any other epic here;
// only the Plans button knows the label.
    ...
    <div class="overflow-x-auto rounded-box border border-base-content/25 bg-base-100 shadow-sm" data-testid="issue-list">
      ...
      {rows.length > 0 && (
        <table class="table text-sm">
          ...
          <tbody>
            {rows.map((s) => (
              <Row key={s.id} sourceId={sourceId} s={s} search={search} />
            ))}
          </tbody>
        </table>
      )}
...
/** Epic progress: closed children over all children. Shared by the list rows
