...
import { encodeIssueQuery, labelStats, sourceById, useIssueListing, useSources } from "@/api/issues.js";
import { tagToken } from "@/api/query.js";
import { shortTimeMs } from "@/lib/format.js";
...
export function LabelsPage({ sourceId = "" }: { sourceId?: string }) {
  ...
  const { data, isLoading } = useIssueListing(id);
  ...
  const named = labelStats(data?.issues ?? []).filter((l) => l.name.toLowerCase().includes(needle));
...
function Row({
  ...
  open: number;
  closed: number;
  /** Unix ms, as labelStats reports it. */
  updatedAt: number;
}) {
  ...
  return (
    <tr class="hover">
      ...
      <td class="whitespace-nowrap text-xs opacity-70">{shortTimeMs(updatedAt)}</td>
    </tr>
  );
}
