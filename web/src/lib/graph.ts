// Dependency graph as mermaid text: the detail page's neighbourhood emits a
// `flowchart LR` from ListDependencies edges and draws it with the same
// mermaid pass documents use, so no layout library is added.
//
...
import { type IssueEdge, IssueStatus } from "@/api/gen/ngicks/crabswarm/issues/v1/issues_service_pb.js";
...
const STATUS_CLASS: Record<IssueStatus, string> = {
  [IssueStatus.UNSPECIFIED]: "st_unknown",
  [IssueStatus.OPEN]: "st_open",
  [IssueStatus.IN_PROGRESS]: "st_in_progress",
  [IssueStatus.BLOCKED]: "st_blocked",
  [IssueStatus.CLOSED]: "st_closed",
  [IssueStatus.DEFERRED]: "st_deferred",
};
...
const CLASS_DEFS = [
  ...
  "classDef st_deferred fill:#ede9fe,stroke:#7c3aed,stroke-width:1.5px,color:#111827",
];
...
export function flowchart(nodes: GraphNode[], edges: IssueEdge[]): Flowchart {
  ...
  nodes.forEach((n, i) => {
    ...
    lines.push(`  ${id}("<b>${label(shorten(n.title))}</b><br/><small>${label(n.id)}</small>"):::${STATUS_CLASS[n.status] ?? "st_unknown"}`);
