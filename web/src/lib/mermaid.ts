...
// rendered markdown (DocumentView, MarkdownField) and the dependency
// neighbourhood (IssueGraph). The SPA's own bundled mermaid, loaded on first
// use, no CDN, securityLevel strict. The theme is baked into the SVG, so
// callers re-run this when the theme flips.
